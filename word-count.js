...
function isValidListNode(node) {
    ...
    if (node.parentElement?.previousElementSibling?.firstChild?.innerText == "Enlaces externos") {
        return false;
    }
...
function toggleWordCount() {
    if (mw.config.get('wgAction') == 'edit') {
        ...
        if (wikiPreviewStyle.display === 'none') {
            alert("Tiene que previsualizar el texto para que el contador de palabras funcione en modo edición.");
...
function getStatistics(bodyContent) {
    ...
    header.innerHTML = "<br/><b>Contador de palabras</b> (<a href='https://es.wikipedia.org/wiki/Usuario:Nacaru/word-count.js'>script</a>)<b>:</b>";
    ...
    main_body_value.innerHTML = "Cuerpo del texto: " + main_body_count + " palabras";
    ref_value.innerHTML = "Referencias: " + ref_count + " palabras";
    total_value.innerHTML = "Total: " + (main_body_count + ref_count) + " palabras";
}

jQuery(function () {
    mw.loader.using(['mediawiki.util'], function () {
        if ($.inArray(mw.config.get('wgAction'), ['edit', 'view', 'submit', 'historysubmit', 'purge']) !== -1) {
            $(mw.util.addPortletLink('p-tb', '#', 'Contar palabras', 't-word-count', 'Contar las palabras del artículo'))
